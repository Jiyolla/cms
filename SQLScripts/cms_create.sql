...
CREATE TABLE IF NOT EXISTS `META_LOG_FILE` (
  ...
  PRIMARY KEY (`ID`),
  CONSTRAINT `FK_META_LOG_FILE_AREA` FOREIGN KEY (`AREA_ID`) REFERENCES `AREA` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_META_LOG_FILE_CCTV` FOREIGN KEY (`CCTV_ID`) REFERENCES `CCTV` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS `LINKS` (
  ...
  PRIMARY KEY (`SEQUENCE_ID`, `ADJACENT_AREA_PathName`),
